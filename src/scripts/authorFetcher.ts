/**
 * Script to fetch authors from Sanity and save to local JSON cache file
 * Run this with: npm run sync-authors
 */
...
async function fetchAuthors(): Promise<void> {
  console.log("\n" + "=".repeat(80));
  console.log("🔄 Fetching authors from Sanity CMS");
  ...
  try {
    ...
    console.log("📡 Querying Sanity for authors...\n");
    const authors = await client.fetch(query);

    if (!authors || authors.length === 0) {
      console.log("⚠️  No authors found in Sanity\n");
      return;
    }
    ...
    // Save to JSON file
    const cachePath = getAuthorCachePath();
    ...
    authors.forEach((author: any, index: number) => {
      const slug = author.slug ? ` (slug: ${author.slug})` : "";
      const email = author.email ? ` - ${author.email}` : "";
      console.log(`  ${index + 1}. ${author.name || "Unnamed"}${slug}${email} - ID: ${author._id}`);
    });
    console.log("\n");

    logger.info(`Fetched ${authors.length} authors from Sanity`);
  } catch (error) {
    console.log("\n❌ Error fetching authors:\n");
    logger.error("Error fetching authors:", {
