...
function parseConfig(argv: string[]): WorkerConfig {
  ...
  const rawLimit = parseInt(process.env.GUNNER_LIMIT || String(DEFAULT_LIMIT), 10);
  ...
  const refreshSanityCache =
    process.env.GUNNER_REFRESH_SANITY_CACHE !== "false" &&
    process.env.GUNNER_REFRESH_SANITY_CACHE !== "0";
...
async function selectCandidates(config: WorkerConfig): Promise<ProcessedCandidateRow[]> {
  if (!config.candidateId) {
    return getProcessedCandidates(config.limit, config.sourceKeys);
  }
