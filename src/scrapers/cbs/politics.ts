...
export async function scrapeCBSPolitics(limit: number = 5): Promise<void> {
  ...
  for (const [index, item] of items.entries()) {
    try {
      ...
      articles.push(
        buildScrapedArticleSuccess(details, "cbsPolitics", item) as typeof articles[number]
      );
      ...
      articles.push(
        buildScrapedArticleFailure("cbsPolitics", item, item, errorMessage) as typeof articles[number]
      );
