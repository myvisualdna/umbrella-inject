...
import { discoverYahooNewsSectionArticles } from "../utils/yahooDiscovery";
...
export async function scrapeYahooUSNews(
  ...
  return discoverYahooNewsSectionArticles($, usNewsUrl, "us", limit);
}
...
export async function scrapeYahooWorldNews(
  ...
  const $ = cheerio.load(response.data);
  return discoverYahooNewsSectionArticles($, worldNewsUrl, "world", limit);
}
...
export async function scrapeYahooPoliticsNews(
  ...
  const $ = cheerio.load(response.data);
  return discoverYahooNewsSectionArticles($, politicsNewsUrl, "politics", limit);
}
