...
export interface ABCNewsUSArticleItem {
  title: string;
  url: string;
  imageUrl?: string;
}

export interface ABCNewsUSArticleDetails {
  url: string;
  title: string;
  excerpt: string;
...
/**
 * Scrapes ABC News US section (https://abcnews.go.com/US)
 * and returns the first 5 articles from the "Top U.S. Stories" block.
 */
export async function scrapeABCNewsUSHomepage(
  ...
  const items: ABCNewsUSArticleItem[] = [];
  ...
  const storyPattern1 = /"title":"([^"]+)","description":"[^"]*","type":"story","location":"(https:\/\/abcnews\.go\.com\/US\/[^"]+)"[^}]*"image":"([^"]+)"/g;
  let match;
  ...
  while ((match = storyPattern1.exec(html)) !== null && items.length < limit) {
    ...
    if (url && url.includes("/US/story") && !seenUrls.has(url) && title && title.length > 10) {
      seenUrls.add(url);
  ...
  if (items.length < limit) {
    const storyPattern2 = /"title":"([^"]+)"[^}]*"type":"story"[^}]*"location":"(https:\/\/abcnews\.go\.com\/US\/[^"]*\/story[^"]*)"/g;
    let match2;
  ...
  if (items.length < limit) {
    const storyPattern3 = /"type":"story"[^}]*"location":"(https:\/\/abcnews\.go\.com\/US\/[^"]*\/story[^"]*)"[^}]*"title":"([^"]+)"/g;
    let match3;
    ...
    $('a[href*="/US/story"]').each((_, link) => {
      ...
      try {
        url = rawHref.startsWith("http")
          ? rawHref
          : new URL(rawHref, abcNewsUSUrl).toString();
      } catch {
        return;
      }

      // Only accept abcnews.go.com/US/story URLs
      if (!url.includes("abcnews.go.com/US/story")) {
        return;
      }
...
/**
 * Scrapes an ABC News article detail page.
...
export async function scrapeABCNewsUSArticleDetails(
  articleUrl: string
): Promise<ABCNewsUSArticleDetails> {
