...
export interface CBSUSArticleItem {
  title: string;
  url: string;
  imageUrl?: string;
}

export interface CBSUSArticleDetails {
  url: string;
  title: string;
  excerpt: string;
...
/**
 * Scrapes CBS News US section (https://www.cbsnews.com/us/)
 ...
 */
export async function scrapeCBSUSNews(
  limit = 5
  ...
  const items: CBSUSArticleItem[] = [];
  const seenUrls = new Set<string>();

  // CBS News US page structure - articles are in links with class item__anchor
  // Headlines are in h4.item__hed inside those links
  $('a.item__anchor').each((_, link) => {
    ...
    try {
      url = rawHref.startsWith("http")
        ? rawHref
        : new URL(rawHref, cbsUSUrl).toString();
    } catch {
 ...
 * - body text
 */
export async function scrapeCBSUSArticleDetails(
  articleUrl: string
): Promise<CBSUSArticleDetails> {
  ...
  if (!category) {
    ...
    if (!category) {
      ...
      if (urlMatch && urlMatch[1] && urlMatch[1] !== "us") {
