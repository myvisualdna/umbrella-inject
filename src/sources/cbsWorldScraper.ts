...
export interface CBSWorldArticleItem {
  title: string;
  url: string;
  imageUrl?: string;
}

export interface CBSWorldArticleDetails {
  url: string;
  title: string;
  excerpt: string;
...
/**
 * Scrapes CBS News World section (https://www.cbsnews.com/world/)
 ...
 */
export async function scrapeCBSWorldNews(
  limit = 5
  ...
  const items: CBSWorldArticleItem[] = [];
  const seenUrls = new Set<string>();

  // CBS News World page structure - articles are in links with class item__anchor
  // Headlines are in h4.item__hed inside those links
  $('a.item__anchor').each((_, link) => {
    ...
    try {
      url = rawHref.startsWith("http")
        ? rawHref
        : new URL(rawHref, cbsWorldUrl).toString();
    } catch {
 ...
 * - body text
 */
export async function scrapeCBSWorldArticleDetails(
  articleUrl: string
): Promise<CBSWorldArticleDetails> {
